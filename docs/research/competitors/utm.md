...
_Method note: getutm.app, docs.getutm.app, mac.getutm.app and apps.apple.com were egress-blocked from the research sandbox, so official docs were read from their public GitHub source repos, release dates come from git tag timestamps in a bare clone of `utmapp/UTM`, and the App Store price is confirmed from user reports in the UTM issue tracker. Facts available only from search snippets of unopenable pages are labelled "(snippet, unverified)"._
...
| Cadence / quality signal | Stable point releases every 1–3 months through 2025; 5.0 betas roughly bi-monthly in 2026. In the v4.7.5 thread a user cited "quality control issues" (three builds for one version); the maintainer replied "shhhh…". The v4.6 notes warned the QEMU 9.1.2 bump "will likely introduce other issues and regressions." | [S1][S18][S20] |
...
- **Display/input path is SPICE.** All I/O goes through SPICE; the host side is **CocoaSpice**, a Metal renderer that composites the guest IOSurface per vblank. On macOS QEMU runs in a separate helper process (`QEMULauncher`) and passes frames by `IOSurfaceID`. [S13]
...
- **x86-64 emulation is not a viable AutoCAD path**: one emulated core by default; users called x86_64 guests "impossible to use" (2021) and TCG boot times run to minutes even for Windows XP (2023). [S9][S14][S38]
...
5. **Fast platform tracking:** Liquid Glass, App Intents, TSO toggle, nested virtualization, ASIF images all landed within months of the Apple releases. [S17][S19][S20]
